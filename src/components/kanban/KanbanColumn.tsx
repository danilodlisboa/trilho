...
import { Plus, MoreVertical, Trash2, Edit3, Check, X } from 'lucide-react';
...
export default function KanbanColumn({ column, cards }: KanbanColumnProps) {
  ...
  return (
    <div className="w-80 shrink-0 bg-slate-900/90 border border-slate-800/80 rounded-2xl flex flex-col max-h-[calc(100vh-7.5rem)] shadow-xl">
      {/* Column Header */}
        ...
        {(provided, snapshot) => (
          <div
            ...
            className={`flex-1 p-3 overflow-y-auto space-y-3 min-h-[150px] transition-colors ${
              snapshot.isDraggingOver ? 'bg-blue-950/20 border-2 border-dashed border-blue-500/40 rounded-xl' : ''
            }`}
          >
            {cards.map((card, idx) => (
